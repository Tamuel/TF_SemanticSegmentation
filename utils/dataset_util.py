...
def run_thread_blocks(number_of_threads, data_list, thread_fn, writer=None, timer=None, return_result=False):
    ...
    :param data_list: List of data shard (ex) [1~1000][1001~2000][2001~3000]
    :param thread_fn: Function for threads
    :param writer: Writer for write to files
    :param timer: Timer for check time of process
    :param return_result: Whether return results or not
    :return: Result list
    """
        ...
        for idx, (img, lab) in enumerate(img_and_label_list):
            ...
            if len(divided_images) > 0:
                for d_i, d_l in zip(divided_images, divided_labels):
                    ...
                    # Write to tfrecord, have to lock writer to avoid conflict
...
def main(unused_argv):
    ...
    make_tfrecord(image_path='D:/Samsung Heavy Industries/DB/original',
                  label_path='D:/Samsung Heavy Industries/DB/LabelsConverted',
                  train_data_output_path='D:/Samsung Heavy Industries/DB/LabelsConverted/SHI_dataset_ori.tfrecord',
                  eval_data_output_path='D:/Samsung Heavy Industries/DB/LabelsConverted/SHI_dataset_eval_ori.tfrecord',
                  shuffle_data=True)
