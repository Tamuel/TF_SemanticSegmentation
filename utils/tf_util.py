...
@add_arg_scope
def fc_layer(inputs, output_channel, bias_initializer=None, weights_regularizer=None, to_batch_norm=False,
             batch_norm_decay=0.997, is_training=True, activation_fn=None, scope=None, return_weights=False):
    with tf.variable_scope(scope, 'fc'):
        input_shape = inputs.get_shape()
        weights = kernels([input_shape[-1], output_channel], regularizer=weights_regularizer, name='weights')
        bias = get_bias(inputs, bias_initializer, 'bias')
...
def separable_multi_conv2d(inputs, filters, basis_rate=list([1, 3, 5]), is_training=True):
    _number_of_basis = len(basis_rate)
    if _number_of_basis < 2:
        raise ValueError('Number of basis_rate must be larger or equal than 2')
    ...
    with slim.arg_scope(
        ...
        with slim.arg_scope([slim.batch_norm], **batch_norm_params):
            ...
            output = split_separable_conv2d(inputs, filters)
            for idx, r in enumerate(basis_rate):
                output += split_separable_conv2d(inputs, filters, rate=r)
            output /= _number_of_basis
...
def multi_conv2d_modified(inputs, filters: tf.Tensor, bias=None,
    ...
    selection_weights3 = kernels([depth, _number_of_basis],
                                 regularizer=slim.l2_regularizer(0.0001),
    ...
    selection += kernels(depth, initializer=tf.truncated_normal_initializer(mean=0, stddev=0.01),
                         regularizer=slim.l2_regularizer(0.0001), name='rate_selection_bias1')
    selection = tf.nn.relu(selection)

    selection = tf.matmul(selection, selection_weights3)
    # selection = batch_norm(selection, is_training, batch_norm_decay)
    selection += kernels(_number_of_basis, initializer=tf.truncated_normal_initializer(mean=0, stddev=0.01),
                         regularizer=slim.l2_regularizer(0.0001), name='rate_selection_bias2')
    selection = tf.nn.relu(selection)
    ...
    for idx, r in enumerate(basis_rate):
        if idx == 0:
            output = atrous_conv2d(inputs, filters, r, bias, padding, stride) * tf.reshape(selection[idx], shape=[-1, 1, 1, 1])
        else:
            output += atrous_conv2d(inputs, filters, r, bias, padding, stride) * tf.reshape(selection[idx], shape=[-1, 1, 1, 1])
...
def global_avg_pooling_layer(inputs, depth=None, to_batch_norm=False, batch_norm_decay=0.997, is_training=True,
                             activation_fn=None, weights_regularizer=None, upsample=True, keepdims=True,
                             robust_to_area=False):
    if not keepdims and upsample:
        raise ValueError('You must set upsample to False if you want to keepdims False')
    if depth is None:
        if robust_to_area:
            output = tf.reduce_sum(inputs, [1, 2], name='global_sum_pooling', keepdims=True)
            bias = tf.reduce_sum(tf.to_float(tf.not_equal(inputs, 0)), [1, 2], name='global_sum_mask', keepdims=True)
            bias = tf.where(tf.equal(bias, 0), tf.ones_like(bias), bias)
            output = output / bias
        else:
