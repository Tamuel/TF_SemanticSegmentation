...
class Segmentator:
    ...
    def network(self, inputs, is_training):
        ...
        with tf.variable_scope('network', reuse=tf.AUTO_REUSE):
            ...
            features = slim.dropout(features, 0.9, is_training=is_training)
            logits = slim.conv2d(features, params['num_classes'], 1,
                                 weights_regularizer=slim.l2_regularizer(FLAGS.weight_decay),
                                 activation_fn=None, normalizer_fn=None, scope='conv_merge')

            rescaled_logits = tf.image.resize_bilinear(logits, size=tf.shape(inputs)[1:3], align_corners=True)
    ...
    def model_fn(self, features, labels, mode):
        ...
        label_weights, gaussian_edge = seg_modify_gradient_weight(labels)
        # loss_logits, selection_mask, loss_image = online_hard_example_mining(logits, one_hot_labels,
        #                                                                      params['num_classes'])
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            with tf.name_scope('Summary'):
                ...
                # tf.summary.image(
                #     'loss_logits',
                #     tf.reduce_sum(tf.reshape(loss_logits, [-1, label_size[1], label_size[2], params['num_classes']]),
                #                   axis=3, keepdims=True),
                #     max_outputs=1
                # )
                # tf.summary.image(
                #     'selection_image',
                #     tf.reshape(selection_mask,
                #                [-1, label_size[1], label_size[2], 1]),
                #     max_outputs=1
                # )
                # tf.summary.image(
                #     'loss_image',
                #     tf.reshape(loss_image,
                #                [-1, label_size[1], label_size[2], 1]),
                #     max_outputs=1
                # )
                tf.summary.image(
