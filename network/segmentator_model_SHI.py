...
class Segmentator:
    ...
    def model_fn(self, features, labels, mode):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            label_mask = tf.tile(tf.cast(tf.not_equal(labels, params['ignore_label']), tf.uint8), [1, 1, 1, 3])
        else:
            label_mask = tf.tile(tf.squeeze(tf.cast(tf.not_equal(labels, params['ignore_label']), tf.uint8), axis=0),
                                 [1, 1, 1, 3])
        predict_decoded_labels *= label_mask
        ...
        gt_decoded_labels = tf.cast(gt_decoded_labels, tf.uint8)

        labels_flat = tf.reshape(labels, [-1])
        preds_flat = tf.reshape(predict_classes, [-1])

        not_ignore_mask = tf.to_float(tf.logical_or(tf.not_equal(labels_flat, params['ignore_label']),
                                                    tf.not_equal(labels_flat, 6)))
        labels_flat = tf.where(
            tf.logical_or(tf.equal(labels_flat, params['ignore_label']), tf.equal(labels_flat, 6)),
            tf.zeros_like(labels_flat), labels_flat)
