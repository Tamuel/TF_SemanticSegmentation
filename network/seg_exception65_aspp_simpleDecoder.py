...
from utils.tf_util import _preprocess_zero_mean_unit_range
from network.xception import xception_71, xception_arg_scope, xception_65
...
class Segmentator:
    ...
    def network(self, inputs, is_training):
        ...
        with tf.name_scope('backbone'):
            ...
            preprocessed_inputs = _preprocess_zero_mean_unit_range(inputs)
            ...
            with slim.arg_scope(xception_arg_scope(
                ...
                logits, end_points = base_model(
                    ...
                    num_classes=None,
                    is_training=(is_training and params['fine_tune_batch_norm']),
                    global_pool=False,
                    keep_prob=1.0,
        ...
        if is_training:
            if params['pre_trained_model'] is not None:
                ...
                variables_to_restore = [
                    ...
                    if params['base_architecture'] in v.name.split(':')[0]
                ]
        ...
        with tf.variable_scope('network', reuse=tf.AUTO_REUSE):
            ...
            # features = pyramid_atrous_convolution(
            #     inputs=logits,
            #     batch_norm_decay=params['batch_norm_decay'],
            #     weight_decay=params['weight_decay'],
            #     is_training=is_training and params['fine_tune_batch_norm'],
            ...
            features_drop_out = slim.dropout(features, keep_prob=0.9, is_training=is_training)

            # raw_features_selection = '/entry_flow/block2/unit_1/xception_module/separable_conv2_pointwise'
            # raw_features = end_points[params['base_architecture'] + raw_features_selection]
            # raw_size = tf.shape(raw_features)
            # decode = refine_by_decoder(features=features_drop_out,
            #                            raw_features=raw_features,
            #                            decoder_height=raw_size[1],
            #                            decoder_width=raw_size[2],
            ...
            decode = simple_decoder(features=features_drop_out,
                                    decoded_height=input_shape[1],
                                    decoded_width=input_shape[2],
                                    number_of_classes=params['num_classes'],
                                    weight_decay=params['weight_decay'])
    ...
    def model_fn(self, features, labels, mode):
        ...
        loss_image = tf.stop_gradient(loss_image)
