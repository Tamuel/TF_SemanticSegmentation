...
from dataprovider.voc2012_provider import voc_classes
...
class Segmentator:
    def __init__(self, params: dict):
        self.network_name = 'Segmentator_scale_conv'
    ...
    def network(self, inputs, is_training):
        ...
        if is_training:
            if params['pre_trained_model'] is not None:
                ...
                variables_to_restore = [
                    ...
                    if params['base_architecture'] in v.name.split(':')[0] and
                       'conv2/bias' not in v.name.split(':')[0] and
                       'input_compress_weights' not in v.name.split(':')[0]
                ]
        ...
        with tf.variable_scope('network', reuse=tf.AUTO_REUSE):
            sspp = self.scale_spatial_pyramid_pooling(
            ...
            resized_tensor = tf.image.resize_bilinear(conv6, size=input_size[1:3], align_corners=True)

        with tf.name_scope('logits'):
            logits = resized_tensor

        return logits

    def scale_spatial_pyramid_pooling(self, inputs, output_stride, batch_norm_decay, is_training, depth=256):
        """Scale Spatial Pyramid Pooling.
        ...
        """
        with tf.variable_scope("sspp", reuse=tf.AUTO_REUSE):
            if output_stride not in [8, 16]:
                raise ValueError('output_stride must be either 8 or 16.')

            atrous_rates = [1.25, 3, 9]
            if output_stride == 8:
            ...
            input_channel = 2048
            stddev = 0.1
            with arg_scope([conv2d, scale_conv2d, atrous_conv2d],
                           batch_norm_decay=batch_norm_decay, to_batch_norm=True,
                           is_training=is_training, activation_fn=tf.nn.relu):
                with arg_scope([kernels], stddev=stddev):
                    feature_list = list()
                    conv_1x1_filter = kernels([1, 1, input_channel, depth], name='aspp_filter1')
                    conv_1x1 = conv2d(
                        inputs, conv_1x1_filter
                    )
                    feature_list.append(conv_1x1)
                    for i, r in enumerate(atrous_rates):
                        conv_3x3_filter = kernels([3, 3, input_channel, depth], name='aspp_filter%d' % (i + 2))
                        conv_3x3 = scale_conv2d(
                            inputs, conv_3x3_filter, initial_step=r
                        )
                        feature_list.append(conv_3x3)

                    # (b) the image-level features
                    with tf.variable_scope("image_level_features", reuse=tf.AUTO_REUSE):
                        # global average pooling
                        image_level_features = tf.reduce_mean(
                            inputs, [1, 2], name='global_average_pooling', keepdims=True
                        )
                        conv_1x1_filter2 = kernels([1, 1, input_channel, depth], name='aspp_filter5')
                        image_level_features = conv2d(image_level_features, conv_1x1_filter2)
                        image_level_features = tf.image.resize_bilinear(
                            image_level_features, inputs_size, name='upsample', align_corners=True
                        )
                        feature_list.append(image_level_features)

                    net = tf.concat(
                        feature_list, axis=3, name='concat'
                    )
                    conv_1x1_filter3 = kernels([1, 1, depth * 5, depth], name='aspp_filter6')
                    net = conv2d(net, conv_1x1_filter3)
    ...
    def model_fn(self, features, labels, mode):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def compute_mean_iou_per_classes(total_cm, name='mean_iou'):
                ...
                # zero division.
                denominator = denominator + tf.ones_like(denominator) * tf.to_float(tf.equal(denominator, 0))
                ...
                result = tf.cond(
                    num_valid_entries > 0,
                    lambda: tf.reduce_sum(iou, name=name) / num_valid_entries,
                    lambda: 0.0
                )
                return result
            ...
            # with tf.name_scope('Summary_probability'):
            #     for i in range(params['num_classes']):
            #         tf.summary.image(
            #             'Class_{}'.format(voc_classes[i]),
            #             tf.reshape(
            #                 tf.transpose(logits, [3, 0, 1, 2])[i],
            #                 [-1, FLAGS.input_height, FLAGS.input_width, 1]
            #             ),
            #             max_outputs=1
            #         )
